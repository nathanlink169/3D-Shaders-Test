...
void GenerateInputEvents()
{
    for( int i=0; i<256; i++ )
    {
        if( g_KeyStates[i] == true && g_OldKeyStates[i] == false )
            g_pGame->HandleInput( InputEvent( InputEventType_Key, InputEventState_Down, i, 0, 0 ) );
        else if( g_KeyStates[i] == true )
            g_pGame->HandleInput( InputEvent( InputEventType_Key, InputEventState_Held, i, 0, 0 ) );
        if( g_KeyStates[i] == false && g_OldKeyStates[i] == true )
            g_pGame->HandleInput( InputEvent( InputEventType_Key, InputEventState_Up, i, 0, 0 ) );
    }

    int mx, my;
    GetMouseCoordinates( &mx, &my );

    g_pGame->HandleInput( InputEvent( InputEventType_Mouse, InputEventState_Max, -1, mx, my ) );

    for( int i=0; i<3; i++ )
    {
        if( g_MouseButtonStates[i] == true && g_OldMouseButtonStates[i] == false )
            g_pGame->HandleInput( InputEvent( InputEventType_Mouse, InputEventState_Down, i, mx, my ) );
        else if( g_MouseButtonStates[i] == true )
            g_pGame->HandleInput( InputEvent( InputEventType_Mouse, InputEventState_Held, i, mx, my ) );
        if( g_MouseButtonStates[i] == false && g_OldMouseButtonStates[i] == true )
            g_pGame->HandleInput( InputEvent( InputEventType_Mouse, InputEventState_Up, i, mx, my ) );
    }
}
