...
inline b2Vec2 b2Contact::GetCollisionNormal()
{
	b2Body* bodyA = GetFixtureA()->GetBody();
	b2Body* bodyB = GetFixtureB()->GetBody();

	b2Vec2 velA = bodyA->GetLinearVelocity();
	b2Vec2 velB = bodyB->GetLinearVelocity();

	b2Vec2 normal = velA - velB;
	normal.Normalize();
